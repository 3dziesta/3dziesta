...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'szablonmoj1'),
        'collapse' => __('collapse child menu', 'szablonmoj1'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'szablonmoj1'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'szablonmoj1'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'szablonmoj1');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'szablonmoj1'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'szablonmoj1'));
}
...
function theme_woocommerce_mini_cart() {
    if (function_exists('woocommerce_mini_cart')) {
        ...
        ob_start(); ?>
        <a class="u-shopping-cart " href="#sec-6200"><span class="u-file-icon u-icon u-shopping-cart-icon u-icon-3"><img src="<?php
                                $header_image = get_header_image();
                                if ($header_image) {
                                    echo esc_url($header_image);
                                } else {
                                    ?><?php echo get_template_directory_uri(); ?>/images/4406139.png<?php
                                }
                            ?>" alt=""><span class="u-icon-circle u-palette-1-base u-shopping-cart-count" style="font-size: 0.75rem;"><!--shopping_cart_count-->0<!--/shopping_cart_count--></span></span>
        </a>
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'szablonmoj1'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'szablonmoj1'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'szablonmoj1'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'szablonmoj1')
    ));
    extract($args);
...
function get_categories_filter_html($args) {
    ...
    // add item all
    $categories_filter_html .= strtr($args['itemTemplate'], array('{categories_filters_content}' => __('All', 'szablonmoj1'), '{categories_filters_value}' => get_permalink(wc_get_page_id('shop'))));
    // add item featured
    $featuredLink = class_exists('Woocommerce') ? add_query_arg('featured', 'true', get_permalink(wc_get_page_id('shop'))) : '';
    $item = strtr($args['itemTemplate'], array('{categories_filters_content}' => __('Featured', 'szablonmoj1'), '{categories_filters_value}' => $featuredLink));
